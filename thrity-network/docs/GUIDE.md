...
│   ├── devtools.py            # Thrity DevTools window (resolver/timing/security/extensions panels)
│   └── extensions/            # the extension system (parser, interpreter, API, loader, tests, CLI)
├── extensions-examples/       # 3 working example extensions, copied into ~/.thrity/extensions on setup
├── examples/
│   └── home.thrity/
│       └── index.html        # a starter website to try out
└── docs/
    ├── GUIDE.md               # this file
    ├── DEVTOOLS.md            # DevTools panels, WebKit inspector
    ├── EXTENSIONS.md          # how to write an extension
    ├── DSL_REFERENCE.md       # the extension language, in full
    ├── EXTENSION_API.md       # every builtin function + event
    └── SERVER_API.md          # host/registry server HTTP endpoints
```
...
- `~/.thrity/extensions/` — installed extensions, one folder each.
  `setup.sh` copies the 3 bundled examples in here the first time
  it's run, if the folder is otherwise empty.
- `~/.thrity/extension-data/` — each extension's private storage
  (one JSON file per extension id), only used if it declares the
  `storage` permission.

---

## 2a. Migrating from v0.04 or earlier

Nothing about your existing `.thrity` sites, registries, or
`hosts.json`/`config.json` needs to change - v0.05 is additive.
Steps to pick up the new features:

1. Replace your `thrity-network/` folder with the new one (or `git
   pull` / re-extract the new tarball over it).
2. Re-run `bash setup.sh` - it's safe to run again, and this time it
   also creates `~/.thrity/extensions/` and
   `~/.thrity/extension-data/`, installing the 3 example extensions
   if you don't already have any installed.
3. Launch 30web as usual. You'll now see extra toolbar buttons from
   the example extensions, plus new **DevTools** and **Inspect**
   buttons.
4. If you were relying on developer extras being *off*
   (`enable_developer_extras`), note it's back on by default in
   v0.05 - see `DEVTOOLS.md`.
5. If you run a registry or host server as a long-lived process
   (systemd service, screen/tmux session, etc.), just restart it with
   the new script - no data migration needed, `registry.json` and
   your site folders are untouched. Add `--verbose` if you want the
   new per-request timing logs.
...
- **Onion (.onion) sites and "Internal SOCKSv5 proxy server error"**:
  as of v0.05, bare `.onion` addresses default to `http://` instead
  of `https://` (most onion services don't speak TLS at all - the
  address itself is already the cryptographic identity), self-signed
  onion certs are trusted automatically the same way `.thrity` certs
  are, and a failed load on an Anonymized tab is retried a couple of
  times automatically before giving up, since a freshly-built Tor
  circuit failing once is common and usually transient. If a
  particular onion address still won't load after those retries:
  confirm Tor is actually running (`systemctl status tor`) and fully
  bootstrapped - the very first connection to a new onion address can
  legitimately take up to a minute even when everything's working.
...
## 12. Developer platform (v0.05+)

- **DevTools**: `DEVTOOLS.md` - WebKit's built-in inspector plus the
  Thrity-specific resolver/timing/security/extensions panels.
- **Extensions**: `EXTENSIONS.md` - full walkthrough of writing,
  testing, and installing an extension.
- **DSL language**: `DSL_REFERENCE.md` - the complete `.thrity`
  extension language.
- **Extension API**: `EXTENSION_API.md` - every builtin function and
  event, and exactly what permission each needs.
- **Server APIs**: `SERVER_API.md` - the host and registry servers'
  HTTP endpoints, including the new debug endpoints.
