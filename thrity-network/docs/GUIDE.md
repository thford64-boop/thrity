...
- **This is plain HTTP, not HTTPS**, meaning traffic isn't encrypted
  — fine for a hobby network of static pages among people who trust
  each other, not appropriate for anything sensitive (passwords,
  personal data, payments). Don't build anything like that on top of
  this without adding real encryption (e.g. a reverse proxy with a
  free Let's Encrypt certificate) first.
