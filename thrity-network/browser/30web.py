...
class ThirtyWeb(Gtk.Window):
    ...
    def on_uri_changed(self, webview, param):
        ...
        if thrity_name:
            # Show the .thrity name instead of the real ip:port, keeping
            # the path/query so deep links still look sensible.
            display = thrity_name + parts.path
            if parts.query:
                display += "?" + parts.query
            self.address_bar.set_text(display)
        else:
